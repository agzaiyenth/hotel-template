      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          {divisions.map((division, idx) => (
            <div
              key={division.number}
              className={`relative bg-black border border-foreground/10 p-8 lg:p-12 hover:border-foreground/30 transition-all duration-500 group ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
              style={{ transitionDelay: isVisible ? `${idx * 100}ms` : "0ms" }}
            >
              <span className="font-mono text-sm text-muted-foreground">{division.number}</span>
              ...
              <div>
                <span className="text-4xl lg:text-5xl font-display">{division.stats.value}</span>
                <span className="block text-sm text-muted-foreground font-mono mt-2">{division.stats.label}</span>
              </div>
            </div>
          ))}
        </div>
